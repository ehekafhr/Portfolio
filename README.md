...
Boostcamp AI Tech에서의 개인 프로젝트로, 리뷰 데이터셋을 통해 별점을 예측하는 모델을 만드는 프로젝트입니다.

데이터를 여러 가지 방향에서 분석하고, 데이터를 전처리하고 여러 가지 기법들을 사용하여 모델을 구축했습니다. 

위의 프로젝트와는 다르게 짧은 시간 동안 진행되었고, 앙상블을 사용한 학습 방식의 특성과 데이터셋의 크기가 크다는 점 때문에 학습 과정이 굉장히 길어졌고, 최선의 결과를 얻지는 못하였습니다.

특히, AdaBoost 등 모델을 개선시키는 알고리즘이 항상 좋은 결과를 내뱉지는 않는다는 것을 깨달았습니다.

또한 이 과정에서 실제 모델을 개발할 때 시간 관리의 중요성과, 버젼 관리를 통한 분석의 중요성을 깨달았습니다.
